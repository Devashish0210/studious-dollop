...
import { addAutoresponderMailbox } from "@/lib/api";
...
const formSchema = z.object({
  ...
  app_registration_clientid: z
    .string()
    .min(1, "App Registration Client ID is required"),
  app_registration_client_secret: z
    .string()
    .min(1, "App Registration Client Secret is required"),
  app_registration_tenantid: z
    .string()
    .min(1, "App Registration Tenant ID is required"),
  last_retrival_timestamp: z.string().datetime().optional(),
});

type FormData = z.infer<typeof formSchema>;

// Props structure for the AddMailbox component
interface AddMailboxProps {
  onSuccess: (newAutoresponder: any) => void;
  onCancel: () => void;
}

// AddMailbox component for adding a new autoresponder mailbox
const AddMailbox: React.FC<AddMailboxProps> = ({ onSuccess, onCancel }) => {
  ...
  const form = useForm<FormData>({
    ...
    defaultValues: {
      ...
      app_registration_clientid: "",
      app_registration_client_secret: "",
      app_registration_tenantid: "",
      last_retrival_timestamp: new Date().toISOString(),
    },
  });
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      // Call the API to add the autoresponder mailbox
      const newAutoresponder = await addAutoresponderMailbox(data);
      // Call the success callback with the new autoresponder data
      onSuccess(newAutoresponder);
    } catch (error: any) {
      console.error("Error creating autoresponder:", error);
      setError(
        error.message || "Failed to create autoresponder. Please try again."
      );
    } finally {
  ...
  return (
    <Form {...form}>
      {/* Form Fields or UI for Add AutoResponder Mailbox */}
