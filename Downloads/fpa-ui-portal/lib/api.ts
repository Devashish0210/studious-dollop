...
export const fetchGlobalSQLTemplates = async (token: string) => {
  try {
    const response = await axios.get(
      `${NEXT_PUBLIC_API_URL}/global-sql-templates`,
      {
        headers: {
          accept: "application/json",
          Authorization: `Bearer ${token}`,
    ...
    console.error("Error fetching global SQL templates:", error);
    return [];
  }
};
