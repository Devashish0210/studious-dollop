...
from dataherald.types import LLMConfig, SQLGeneration, Prompt
...
class SQLGenerationService:
    ...
    def generate_folowup_response_with_timeout(
        self, sql_generator, user_prompt, db_connection, metadata=None
    ):
        return sql_generator.generate_followup_response(
            user_prompt=user_prompt,
            database_connection=db_connection,
            metadata=metadata,
        )
    ...
    def create(  # noqa: PLR0912
        ...
        if sql_generation_request.evaluate:
            ...
            #     database_connection=db_connection,
            # )
            initial_sql_generation.evaluate = False
            initial_sql_generation.confidence_score = 0
    ...
    def execute_invoice_query(self, supplier_name: str, start_date: str, end_date: str, max_rows: int = 100) -> tuple[str, dict]:
        db_connection_id = "68358711417c250621dd3767"
        
        if not supplier_name:
            return "Supplier name must be provided.", {}
        if not start_date:
            return "Start date must be provided.", {}
        if not end_date:
            return "End date must be provided.", {}
        ...
        sql_query = f"SELECT * FROM public.fpa_invoices WHERE \"Supplier\"='{supplier_name}' AND \"InvoiceDate\" >= '{start_str}' AND \"InvoiceDate\" <= '{end_str}';"

        db_connection_repository = DatabaseConnectionRepository(self.storage)
        db_connection = db_connection_repository.find_by_id(db_connection_id)
        database = SQLDatabase.get_sql_engine(db_connection, True)
        return database.run_sql(sql_query, max_rows)
    ...
    def create_followup_sql_generation(
        self,
        prompt:Prompt,
        db_connection,
        database,
        ...
        if sql_generation_request.sql is not None:
            ...
            # Optionally, validate the provided SQL
            try:
                sql_generation = create_sql_query_status(
                    db=database, query=sql_generation.sql, sql_generation=sql_generation
                )
            ...
            try:
                with ThreadPoolExecutor(max_workers=1) as executor:
                    future = executor.submit(
                        self.generate_folowup_response_with_timeout,
                        sql_generator,
                        prompt,
                        db_connection,
                        metadata=langsmith_metadata,
