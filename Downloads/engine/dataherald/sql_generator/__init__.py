...
from dataherald.utils.agent_prompts import CLASSIFIER_PROMPT
...
class TableSelection(BaseModel):
    """Routing result for selecting the appropriate database table."""
    table_names: List[str] = Field(description="The names of the SQL tables that best match the user request.")
...
class SQLGenerator(Component, ABC):
    ...
    def filter_tables_by_schema(
        ...
        myprompt = ChatPromptTemplate.from_messages([
            ...
            ("human", str(prompt.text)),
        ])
        try:
            llm = AzureChatOpenAI(
            azure_deployment=os.getenv("LLM_MODEL", "your-deployment-name"),
            ...
            chain = myprompt | llm | parser
            result = chain.invoke({
                "user_query": str(prompt.text),
                "format_instructions": parser.get_format_instructions()
            })
        except Exception as e :
           logging.error(e,'error from classfication')
        
        if result.table_names:
            print(result.table_names,'specific table name ')
            return [table for table in db_scan if table.table_name in result.table_names]
        print('returning complete database')
        return db_scan
