...
from modules.chat_history import controller as chat_history_controller
from modules.global_sql_template import controller as global_sql_template_controller
...
tags_metadata = [
    ...
    {"name": "Chat History", "description": "chat history endpoints"},
]

app = FastAPI(
    root_path="/copilot/dataheraldenterprise",
    ...
    docs_url="/docs",
    redoc_url="/redoc",
    title="Data Herald Enterprise Documentation",
    description="Backend for the Data Herald Enterprise"
)
...
app.include_router(chat_history_controller.ac_router, tags=["Chat History"])
app.include_router(global_sql_template_controller.router, tags=["Global SQL Templates"])
...
async def engine_heartbeat():
    async with httpx.AsyncClient() as client:
        response = await client.get(settings.engine_url + "/heartbeat", headers={"X-OpenAI-Key": settings.api_key},)
