...
const afterCallback: AfterCallback = async (
  ...
  try {
    ...
    console.error(error)
    res.writeHead(302, {
      Location: `/copilot/dataherald/auth/error?message=${encodeURIComponent(e as string)}`,
    })
    res.end()
  }
  return session
...
export default handleAuth({
  ...
  onError(_: NextApiRequest, res: NextApiResponse, error: HandlerError) {
    console.error(error)
    res.writeHead(302, {
      Location: `/copilot/dataherald/auth/error?message=${encodeURIComponent(error.message)}`,
    })
    res.end()
  },
})
