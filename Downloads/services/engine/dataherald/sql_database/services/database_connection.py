...
class DatabaseConnectionService:
    ...
    def _assert_driver_port_match(self, connection_uri: str):
        """Simple heuristic to detect driver vs port mismatches and give a clear error."""
        try:
            prefix = DatabaseConnection.get_dialect(connection_uri).lower()
        except Exception:
            return

        # common mismatches:
        if "postgres" in prefix and ":3306" in connection_uri:
            raise ValueError(
                "Connection URI uses a Postgres driver/scheme but a MySQL port (3306). "
                "If this is Aurora MySQL, use 'mysql+pymysql://...' as the URI scheme."
            )
        if "mysql" in prefix and ":5432" in connection_uri:
            raise ValueError(
                "Connection URI uses a MySQL driver/scheme but a Postgres port (5432). "
                "If this is Aurora PostgreSQL, use 'postgresql+psycopg2://...' as the URI scheme."
            )
        # no-op if nothing suspicious found
        return
