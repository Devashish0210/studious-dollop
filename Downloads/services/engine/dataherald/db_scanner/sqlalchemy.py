...
class SqlAlchemyScanner(Scanner):
    ...
    def get_processed_column(  # noqa: PLR0911
        ...
        try:
            ...
            logger.warning(f"Timeout/error fetching sample from {table}.{column['name']}: {str(e)}")
            # Return basic column info without cardinality check
            return ColumnDetail(
                name=column["name"],
        ...
        try:
            ...
            logger.warning(f"Cardinality check failed for {table}.{column['name']}: {str(e)}")
    ...
    def scan_single_table(
        ...
        # # Sanitize table name (ensure it's a str and strip whitespace / control chars)
        # raw_table = table
        # try:
        #     table_name = str(raw_table).strip()
        # except Exception:
        #     logger.warning("Skipping table with non-convertible name: %r", raw_table)
        #     return

        # if table_name == "":
        #     logger.warning("Skipping empty table name (original: %r)", raw_table)
        #     return

        # logger.info("Scanning table: %s", table_name)
        # try:
        #     columns = inspector.get_columns(table_name=table_name)
        # except Exception as e:
        #     # Reflection failed (missing privileges, view references, or no such table).
        #     logger.warning(
        #         "Skipping table %s due to reflection error: %s", table_name, str(e)
        #     )
        #     return
    ...
    def scan(
        ...
        for table in table_descriptions:
            try:
                ...
                logger.error(f"Failed to scan table {table.table_name}: {str(e)}")
                repository.save_table_info(
                    TableDescription(
                        db_connection_id=table.db_connection_id,
                        table_name=table.table_name,  # Use .table_name, not table object
            ...
            try:
                logger.info(f"Get logs table: {table.table_name}")
